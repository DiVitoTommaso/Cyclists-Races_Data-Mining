...
class DataCleaner:
    ...
    def describe(self):
        ...
        for column in self.df.columns:
            ...
            print(
                f"{column:<30} | {non_null_count:<15} | {total:<15} | {total != non_null_count}"
            )
    ...
    def get_birth_distributions(self):
        ...
        nationality_distributions = self.cyclist_df.groupby("nationality")[
            "birth_year"
        ].describe()
    ...
    def get_age_distributions(self):
        overall_distribution = self.cyclist_df.groupby("cyclist_age")[
            "cyclist_age"
        ].describe()
        nationality_distributions = self.cyclist_df.groupby(
            ["cyclist_age", "nationality"]
        )["cyclist_age"].describe()
    ...
    def get_climb_distributions(self):
        profile_distributions = self.cyclist_df.groupby("profile")[
            "climb_total"
        ].describe()
    ...
    def fix_cyclist(self):
        ...
        for c in cyclists:
            ...
            for _, row in self.df.iterrows():
                row["cyclist_age"] = datetime.strptime(
                    row["date"], "%Y-%m-%d %H:%M:%S"
                ) - datetime.strptime("1995-08-05", "%Y-%m-%d")
